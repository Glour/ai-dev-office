...
task_file=""
artifacts_dir="${AI_OFFICE_ARTIFACTS_DIR:-./artifacts}/codex-runs"
...
if [ "$dry_run" -eq 1 ]; then
  ...
  if ! command -v "$codex_bin" >/dev/null 2>&1; then
    ...
    set +e
    "$codex_bin" "$(cat "$task_file")" > "$run_dir/stdout.log" 2> "$run_dir/stderr.log"
    code=$?
    set -e
